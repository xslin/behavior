(function () {
    ...
    var BuriedPoint = {
        isInit: false,
        URL: {
            serverURL: "http://127.0.0.1:18090/behavior/ea.gif"//生产环境外网地址
        },
        /**
         * 构造get请求参数
         * @param params
        ...
        getReferrer: function () {
            ...
            return Utils.http.getUrl(referrer);
        },
        // 识别客户终端类型
                ...
                while ((param = _maq.shift())) {
                    switch (param[0]) {
                        ...
                        case '_trackPageview':
                            ...
                            break;
                        case '_trackEvent':
                            params.ec = Utils.trim(param[1],"g") || ''; // 类型分类
                            params.ea = Utils.trim(param[2],"g") || ''; // 动作
                            if (param[3]) {
                                params.el = Utils.trim(param[3],"g") || ''; // 事件标签
                            }
                            if (param[4]) {
                                params.ev = Utils.trim(param[4],"g") || ''; // 事件数值信息
                            }
                            this.sendData();
                            break;
        ...
        initParams: function () {
            ...
            if (document) {
                ...
                params.ul = encodeURIComponent(document.URL && document.URL || ''); // 访问的URL
                params.te = document.title || ''; // 页面title
                params.re = encodeURIComponent(this.getReferrer() || ''); // 从哪个路径过来
            }
            // Window对象数据
            if (window && window.screen) {
                params.shw = (window.screen.width || 0) + "X" + (window.screen.height || 0);
            ...
            params.sf = this.getSource();
            params.uvi = this.getUvi();
            this.isInit = true;
        },
        ...
        getUk: function () {
            var _utmb = Utils.Cookie.getCookie("_utmb");
            if (!_utmb || _utmb == 'undefined') {
                _utmb = Math.uuidFast();
                Utils.Cookie.setCookie("_utmb", _utmb);
            }
            return _utmb;
        },
        // 检查是否刷新过于频繁
        ...
        sendData: function () {
            ...
            img.src = BuriedPoint.URL.serverURL + "?" + queryString;
        ...
        buttonAddEvent: function () {
            ...
            for (var i = 0; i < btns.length; i++) {
                ...
                this.addEvent(btns[i], 'click', function () {
                    var linxshData = this.getAttribute('linxsh-data') || ''; //ev
                    var ec = this.getAttribute('linxsh-ec') || 'AUTO'; //ev
                    var ea = this.getAttribute('linxsh-ea') || this.innerText; //ev
                    var el = this.getAttribute('linxsh-el') || ''; //ev
                    _maq.push(['_trackEvent', Utils.trim(ec,"g"), Utils.trim(ea,"g"), Utils.trim(el,"g"), Utils.trim(linxshData,"g")]);
                });
                // }
            }
        },
        ...
        aAddEvent: function () {
            ...
            for (var i = 0; i < as.length; i++) {
                ...
                this.addEvent(as[i], 'click', function () {
                    ...
                    var ea = this.getAttribute('linxsh-ea') || this.innerText; //ev
                    var el = this.getAttribute('linxsh-el') || ''; //ev
                    _maq.push(['_trackEvent', ec.trim(), ea.trim(), el.trim(), linxshData.trim()]);
                    _maq.push(['_trackEvent', Utils.trim(ec,"g"), Utils.trim(ea,"g"), Utils.trim(el,"g"), Utils.trim(linxshData,"g")]);
                });
                // }
            }
        ...
        setFi: function () {
            ...
            if (referrer) {
                params.fi = Utils.md5(referrer);
            } else {
                params.fi = "-";
            }
        },
        getUvi: function () {
            var uvi = Utils.Cookie.getCookie("_uvi") || '';
            if(uvi!=''){
                return uvi;
            }
            if (Utils.device.isMac() || !Utils.device.isPc()) {
                if (!uvi || uvi == "undefined") {
                    uvi = Math.uuidFast();
                    Utils.Cookie.setCookie("_uvi", uvi, 60 * 60 * 24 * 365 * 10);
                }
            }
            return uvi;
        }
